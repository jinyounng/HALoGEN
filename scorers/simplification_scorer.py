'''
This module contains the simplification scorer class.
    ...
    def compute_scores(self):
        ...
        for each_sample in self.responses[1:]:
            ...
            if abstention or each_sample[columns.index("response")].strip()=="":
                ...
                hallucinated_atoms = [x.replace("| ", "") for x in each_sample[columns.index("hallucinated_atomic_units")].split("|\n") if x and x != ""]
...
if __name__ == "__main__":
    ...
    for each_response_file in glob.glob("../model_hallucinations/simplification/*.csv"):

        model_name = each_response_file.split("_simplification")[0].split("/")[-1]


        summ_scorer = SimpScorer(each_response_file)
        h_score, r_ratio, u_score = summ_scorer.compute_scores()
        print("===="+model_name+"====")
